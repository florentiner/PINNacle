    ...
    for traj in range(train_args["n_trajectories"]):
        ...
        for t in itertools.count():
            ...
            if done == 1:
                ...
                # Kmax из статьи (формула 11): цепочка не длиннее
                # max_chain_length СТАДИЙ. rl_penalty=-1 действует не сразу, а
                # на следующем шаге: env выставит done=-1, когда стадия уже
                # выполнена. Поэтому взводить пенальти надо на шаг раньше —
                # после (Kmax-1)-й стадии, чтобы терминальной оказалась
                # Kmax-я. С условием (t + 1) >= max_chain_length цепочка
                # получала Kmax+1 стадий: при Kmax=10 провалы шли по 11 шагов,
                # и 7 успехов из 955 в волне v8 случились на 11-й стадии,
                # которой по статье быть не должно.
                if (t + 2) >= max_chain_length:
                    rl_penalty = -1
            elif done == -1:
                rl_penalty = 0
                break
