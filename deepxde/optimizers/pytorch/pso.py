...
class PSO(torch.optim.Optimizer):
    ...
    def __init__(self,
        ...
        self.v = self.start_velocities(device)
        self.m1 = torch.zeros(self.pop_size, self.vec_shape, device=device)
        self.m2 = torch.zeros(self.pop_size, self.vec_shape, device=device)
    ...
    def start_velocities(self, device=None) -> torch.Tensor:
        ...
        """
        return torch.zeros((self.pop_size, self.vec_shape), device=device)
    ...
    def get_randoms(self) -> torch.Tensor:
        ...
        """
        device = next(iter(self.params)).device
        return torch.rand((2, 1, self.vec_shape), device=device)
