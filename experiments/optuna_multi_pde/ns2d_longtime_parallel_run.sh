#!/bin/bash
# Automatic launch of ns2d_longtime_optuna.py with GPU distribution via nvidia-smi
...
SCRIPT="experiments/optuna_multi_pde/ns2d_longtime_optuna.py"
# Absolute path so every worker and optuna-dashboard (any cwd) resolve the same file.
DB_PATH="${SCRIPT_DIR}/optuna_studies/ns2d_longtime.db"
STUDY_NAME="ns2d_longtime_chain_continues_1"
N_TRIALS="${1:-15}"  # per process; 8 processes => 8 * 8 = 64 total trials
N_PROCESSES="${2:-4}"  # Default to 8 processes, can be overridden
...
# SQLAlchemy/SQLite absolute URL (four slashes after sqlite:) — use this string for optuna-dashboard.
echo "Dashboard storage URL: sqlite:////${DB_PATH#/}"
...
if [ "${NUM_GPUS:-0}" -gt 0 ]; then
    ...
    for i in $(seq 0 $((N_PROCESSES - 1))); do
        gpu=$((i % NUM_GPUS))
        CUDA_VISIBLE_DEVICES=$gpu $PYTHON "$SCRIPT" --db-path "$DB_PATH" --study-name "$STUDY_NAME" --sampler tpe --n-trials "$N_TRIALS" &
    done
else
    echo "No CUDA GPUs via nvidia-smi. Falling back to CPU ($N_PROCESSES processes)..."
    for _ in $(seq 1 "$N_PROCESSES"); do
        $PYTHON "$SCRIPT" --db-path "$DB_PATH" --study-name "$STUDY_NAME" --sampler tpe --n-trials "$N_TRIALS" &
    done
fi
