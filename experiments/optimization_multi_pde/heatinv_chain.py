...
proj_name = "rlpinn-heatinv-optimization"
experiment = start(
...
experiment.log_parameters({
    "param": "v_1",
    "reward_function": "v_2",
    "description": "optimization_heatinv_basic_RL_optimizer"
})
...
def build_get_model_heatinv():
    def get_model():
        pde = HeatInv()
        # Use the PDE's recommended PFNN architecture (split inputs for u/a)
        ...
        for i, c in enumerate(pde.loss_config):
            t = c.get("type", "")
            if t in ("boundary", "initial", "ic", "pointset"):
...
def main():
    ...
    get_model = build_get_model_heatinv()
    get_model_rec = build_get_model_heatinv()
    ...
    rl_agent_params = {
        ...
        "exp": experiment,
        "log_key": args.log_key,
        "proj_name": "rlpinn-heatinv-farm-transitions",
    }
