...
## 6. Аудит честности сравнения (2026-07-27, перед финальными прогонами)

Проверено по коду и данным:

1. **Одинаковая инициализация.** Воркер ставит `dde.config.set_random_seed(seed)`
   ДО построения модели и данных; затем одинаковый xavier-reinit. Путь кода общий
   для всех оптимизаторов ⇒ при одном сиде все методы стартуют с одних весов и
   одних точек коллокации. Детерминизм доказан эмпирически: два независимых
   прогона одной цепочки (heatinv continuous vs «fixed»-дубль) дали бит-идентичные
   метрики на всех 10 сидах.
2. **Нет утечки данных.** Обучение видит только PDE-точки (сэмплируются из
   домена) и BC/IC; референсные решения `ref/*.dat` читает исключительно
   TesterCallback для вычисления метрик (model.predict), в обучение они не
   попадают и на ход обучения callback не влияет.
3. **Одинаковые лоссы и веса**: boundary/initial=100, pde=1 — один билдер для всех.
4. **Одинаковый бюджет**: у каждого метода ровно 31 000 шагов оптимизатора
   (Adam1000+LBFGS30000 / SOAP31000 / Muon31000 / Adam1000+X30000); одинаковый
   display_every=100 ⇒ одинаковый оверхед валидации. Wall-clock у SOAP ~1.5–2×
   на шаг — elapsed_s пишется в каждую строку, так что сравнение можно читать и
   в шагах, и во времени.
5. **Изоляция прогонов**: каждый сид — отдельный subprocess со свежей моделью;
   никаких тёплых стартов и общего состояния.
6. **Провенанс**: полный конфиг цепочки лежит в chain_json каждой строки;
   resume/merge ключуются (pde, chain_key, smoke, seed) — строки soap /
   adam_soap / lr-sweep не смешиваются.
7. **Гиперпараметры из первоисточников**: SOAP lr=3e-3 (дефолт официального
   кода), precondition_frequency=2 (оптимум абляций NeurIPS-статьи); Muon
   lr=0.02, momentum=0.95, ns_steps=5, aux-AdamW 3e-4 (дефолты README; те же
   значения в SpecMuon для PINN). Плюс собственный lr-sweep (burgers: SOAP
   {1e-3, 3e-3, 1e-2}, Muon {5e-3, 2e-2, 5e-2}) — эмпирическая защита от
   претензии «взяли плохие параметры».

Известные оговорки (честно указать в тексте статьи): бюджет-матчинг по шагам
даёт SOAP больше wall-clock; разбиение параметров Muon — по README (скрытые
матрицы в Muon, первый/последний слой + bias в aux-AdamW), альтернативные
разбиения не перебирались.
